...
public class HandleSurgeryWithoutPlayer {
    public static void execute(LivingEntity player, IItemHandlerModifiable table, ItemStackHandler body) {
        ...
        for(ItemStack stack : ejectList) {
            if (!stack.isEmpty()) {
                ItemEntity itemEntity = HandleCyberwareUserDataWithoutPlayer.drop(player, stack);
